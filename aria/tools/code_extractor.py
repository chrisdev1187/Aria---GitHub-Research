...
MAX_FILE_SIZE_CHARS = 30_000  # Truncate files longer than this
...
class CodeExtractor:
    ...
    async def extract_repo_files(
        ...
        for entry in tree:
            ...
            if isinstance(size, (int, float)) and size > MAX_FILE_SIZE_CHARS:
                continue
        ...
        for item in selected:
            try:
                ...
                if content and len(content) > 20:  # Skip empty/tiny files
                    results.append({
                        "path": item["path"],
